...
int main()
{
    std::string testTitle("Initized using Array: No Failure Expected");

    std::clog << "\n" << testTitle << std::endl;
    std::cout<< "<CTestLabel>" << testTitle << "</CTestLabel>" << std::endl;

    GDTestDataArray positivePathArray = copyVecToArray(testPostivePathVec);
    ...
    try
    {
        ...
        if (checkID.has_value())
        {
            ...
            {
                std::clog << "dictionary.lookupID(\"Functional Test Str 0\") PASSED\n";
            }
        }
        else
        {
        ...
        if (checkName.has_value())
        {
            ...
            {
                std::clog << "dictionary.lookupName(TestFunctionalityEnumDict::FunctionalTest_15) PASSED\n";
            }
        }
        else
        {
